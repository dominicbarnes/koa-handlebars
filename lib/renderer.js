...
/**
 * Processes a map of partial keys with values that may be inline partial
 * template strings, or pre-compiled Handlebars templates, returning a map
 * of compiled Handlebars templates, ready to render.
 *
 * @yield {Object} Shallow hash of partials, processed into objects with render
 * functions.
 */
Renderer.prototype.processPartialMap = function (partialMap) {
  var self = this;
  var processedMap = {};
  for (var key in partialMap) {
    if (!partialMap.hasOwnProperty(key)) {
      continue;
    }

    var partialValue = partialMap[key];
    var meta;
    if (typeof partialValue === 'string') {
      processedMap[key] = self.handlebars.compile(partialValue);
    } else if (typeof partialValue === 'function') {
      processedMap[key] = partialValue;
    }
  }
  return processedMap;
};
...
Renderer.prototype.render = function *(template, locals, options) {
  ...
  // Pass through local helpers or partials if they exist
  if (locals.partials) {
    locals.partials = this.processPartialMap(locals.partials);
    options.partials = extend(true, options.partials, locals.partials);
  }
  if (locals.helpers) {
    options.helpers = extend(true, options.helpers, locals.helpers);
  }
